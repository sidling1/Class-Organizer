...
from .models import ChatGroup, Dept, Class, Student, Attendance, Course, Teacher, Assign, AttendanceTotal, time_slots, \
...
def groups(request, stud_id):
    ...
    for ass in ass_list:
        try:
            grp = ChatGroup.objects.get(course = ass.course)
            grps_list.append(grp)
        except:
            print("Some Error Occured", ass.course)


    context = {'group_list': grps_list}
    
    return render(request, 'info/groups.html', context)


@login_required 
def chat_view(request, stud_id , course_id):
    course = Course.objects.get(id=course_id)
    group = ChatGroup.objects.get(course = course)
    messages = ChatMessage.objects.filter(group=group).order_by('timestamp')
    context = {
        'group': group,
        'messages': messages,
        'user': stud_id
        # 'user': request.user,  # Assuming you have authentication setup
    }
    return render(request, 'info/chat.html', context)
